...
export default function EvaluationsPage() {
  ...
  const loadMockData = () => {
    ...
    const mockEvaluations: Evaluation[] = [
      ...
      },
      {
        ...
        status: "active",
        version: 2,
        previousVersionId: "prev-version-id", // Just for demonstration
  ...
  const handleCreateEvaluation = () => {
    ...
    const evaluation: Evaluation = {
      ...
      version: 1, // Initialize as version 1
    }
  ...
  const cloneEvaluation = (evaluation: Evaluation) => {
    // Use the version property instead of parsing from title
    const currentVersion = evaluation.version || 1
    ...
    const cloned: Evaluation = {
      ...
      createdAt: new Date(),
      version: newVersion,
      previousVersionId: evaluation.id,
    }

    const updated = [...evaluations, cloned]
    setEvaluations(updated)
    localStorage.setItem("evaluations", JSON.stringify(updated))
    writeAudit(`Evaluation cloned: ${evaluation.title} (v${currentVersion} → v${newVersion})`)
    alert(`Created new version of "${evaluation.title}" (v${newVersion})`)
  }
  ...
  const handleCloseViewCandidates = () => {
